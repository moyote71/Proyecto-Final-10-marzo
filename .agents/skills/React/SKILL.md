...
description: Guía de desarrollo frontend con React 18+. Cubre setup con Vite, estructura de proyecto, hooks, routing con React Router, Context API, patrones de componentes, optimización de performance e integración con APIs backend.
scope: frontend
...
// Ejecutar cuando cambia dependencia
useEffect(() => {
  fetchData(userId);
}, [userId]);
...
useEffect(() => {
  if (isAuthenticated && userId) loadUserData();
...
*Versión: 1.0.0 | Scope: frontend | Stack: React 18+ / Vite / React Router*
